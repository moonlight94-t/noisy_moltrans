...
class BIN_Interaction_Flat1(nn.Sequential):   
    ...
    def forward(self, d, p, d_mask, p_mask):
        ...
        ex_d_mask = d_mask.unsqueeze(1).unsqueeze(2) # (batch_size, hidden_size) -> (batch_size, 1, 1, hidden_size) / hidden_size가 아니라 seq_length가 맞는 것 같은데 
...
class BIN_Interaction_Flat2(nn.Sequential):   
    def __init__(self, **config):
        ...
        #encoder
        self.n_layer = 5 # encoder layer 개수 
        self.hidden_size = config['emb_size']
        self.intermediate_size = config['intermediate_size']
        self.num_attention_heads = config['num_attention_heads']
        ...
        self.icnn = nn.Conv2d(64, 3, 3, padding = 0) # in_channel size, out_channel size, kernel size
        self.icnn2= nn.Conv2d(1, 16, 3, padding = 1)
        self.icnn3= nn.Conv2d(16, 64 , 3 , padding = 1)
        #self.icnn4= nn.Conv2d(64, 256, 3, padding= 1)
        #self.icnn5= nn.Conv2d(256, 32 , 3, padding = 1)
    ...
    def forward(self, d, p, d_mask, p_mask):
        ...
        f = self.icnn(self.icnn3(self.icnn2(i_v)))
